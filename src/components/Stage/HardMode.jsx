...
function FillBlankExercise({ item, world, character, label, onAnswer }) {
  ...
  // - 숫자(영어): 한국어 발음 (예: one → 원, nine → 나인)
  // - 알파벳(대/소문자): 한국어 발음 (예: A → 에이, B → 비)
  const hintContent = (() => {
    ...
    if (world.id === 'numbers_en')                             return item.pronunciation  // 원, 투, 쓰리...
    if (world.id === 'alphabet_upper' || world.id === 'alphabet_lower') return item.pronunciation  // 에이, 비, 씨...
    return wordDisplay
  })()
